...
OperationHelper.prototype.search = function(params, callback) {
    var searchCallback = function(err,result,raw){
        var items = [];
        if(!err){
            items = result.ItemSearchResponse.Items[0];
        }
        callback(err, items, raw);
    };
    // OperationHelper.prototype.execute.call(this,'ItemSearch',params,searchCallback);    
    this.execute('ItemSearch',params,searchCallback);    
};
